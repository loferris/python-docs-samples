...
# Needs actual conceptual update for use case
"""INPUTS_SPEC = {
...
OUTPUTS_SPEC = {
    "is_precipitation": tf.TensorSpec(shape=(None, 1), dtype=tf.float32),
}
...
def create_model(train_dataset: tf.data.Dataset) -> keras.Model:
    ...
    output_layers = {
        "is_fishing": sequential_layers(
            ...
            keras.layers.Dense(1, activation="sigmoid", name="is_precipitation"),
        )
    }
...
def run(
    ...
    with distributed_strategy.scope():
        ...
        model.compile(
            ...
            loss={"is_precipitation": "binary_crossentropy"},  # https://keras.io/api/losses
            metrics={"is_precipitation": ["accuracy"]},  # https://keras.io/api/metrics
        )
